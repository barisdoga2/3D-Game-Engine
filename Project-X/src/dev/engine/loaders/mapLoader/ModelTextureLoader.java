		...
		try {
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					boolean transparency = Boolean
							.parseBoolean(eElement.getElementsByTagName("transparency").item(0).getTextContent());
					boolean fakeLighting = Boolean
							.parseBoolean(eElement.getElementsByTagName("fakeLighting").item(0).getTextContent());
					float shineDamper = Float
							.parseFloat(eElement.getElementsByTagName("shineDamper").item(0).getTextContent());
					float reflectivity = Float
							.parseFloat(eElement.getElementsByTagName("reflectivity").item(0).getTextContent());
					...
					all.add(new ModelTexture(name, textureID, transparency, fakeLighting, shineDamper, reflectivity,
							rowSize));
