...
public class SpecularTextureLoader {
	...
	private static final String XML_PATH = "res/xml/specularTextures.xml";

	protected static List<SpecularTexture> loadAll() {
		List<SpecularTexture> all = new ArrayList<SpecularTexture>();
		try {
			...
			NodeList nList = doc.getElementsByTagName("specularTexture");
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					all.add(new SpecularTexture(name, textureID));
